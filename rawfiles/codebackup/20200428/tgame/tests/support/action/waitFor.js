 ...
 */
const waitFor =
  (person, elem, checkElement, otherElement, number, obsolete, ms, isWaitingOnSpecificState, falseState, state, done) => {
    /**
    ...
    if (command === 'waitForExist') {
      browserData.waitUntil(() => {
        return browserData.isVisible(elem) || (otherElement && browserData.isVisible(otherElement));
      }, intMs, `expect element "${elem}" or "${otherElement}" to exist in ${intMs}ms`);
    } else {
      browserData[command](elem, intMs, boolFalseState);
    }
