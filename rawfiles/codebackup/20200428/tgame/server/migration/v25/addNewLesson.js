...
export default function uploadLessonFile() {
  ...
  for (let k = 0; k < poolfiles.length; k++) {
    ...
    // var stats = fs.statSync(process.env.PWD + '/private/NewCourses/Pool/' + poolfiles[k]);
    ...
    // if (sc && sc.filetime && sc.filetime.getTime() == mtime.getTime()) continue;
  ...
  for (let k = 0; k < poolhomeworks.length; k++) {
    ...
    // var stats = fs.statSync(process.env.PWD + '/private/NewCourses/Pool/' + poolhomeworks[k]);
    // var mtime = new Date(util.inspect(stats.mtime));
  ...
  for (let k = 0; k < tankfiles.length; k++) {
    ...
    // var stats = fs.statSync(process.env.PWD + '/private/NewCourses/Tank/' + tankfiles[k]);
    // var mtime = new Date(util.inspect(stats.mtime));
    // console.log(mtime);
    const sc = SlideContent.findOne({_id: f._id });
    // if (sc && sc.filetime && sc.filetime.getTime() == mtime.getTime()) continue;
