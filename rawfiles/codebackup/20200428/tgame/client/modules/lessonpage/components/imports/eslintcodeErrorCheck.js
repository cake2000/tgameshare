...
const keyWords2 = new Set([
  ...
  'Balls', 'BallDiameter', 'TableHeight', 'TableWidth', 'Pockets', 'Rails', 'Cushions', 'CushionWidth', 'PlayerInfo',
  'getAimPosition', 'getNewCommand', 'getRandomNumber', "getRandomCommand", 'extrapolatePoints', "CandidateBallList", 'calculateProbability', 'console', 'MyID', 'world', 'isPathBlocked', 'getCutAngle', 'calculateCutAngle', 'calculateSidePocketSkew', 'getAngleToSidePocket',
  'calculateEndState', 'calculateCTP', 'calculateProbability2', 'getDistance', 'Victor', 'getFirstBallTouched', 'Boundaries', "TOP_Y", "BOTTOM_Y", "LEFT_X", "RIGHT_X", 'debugger',
]);

const initVarList = {'Pockets': "array", "Balls": "array", Tanks: "array", Weapons: "array", "MyTank": "object", "Crystals": "array", "Maze": "array", "ballID": "number", "pocketID": "number", "ball_id": "number", "pocket_id": "number", "BallDiameter": "number"};

const FunctionArgTypes = [];
const FunctionArgMeanings = [];
FunctionArgTypes["isPocketBlockedFromBall"] = ["number", "number"];
FunctionArgMeanings["isPocketBlockedFromBall"] = ["ball ID", "pocket ID"];
FunctionArgTypes["isSidePocketWithLargeSkew"] = ["number", "number"];
FunctionArgMeanings["isSidePocketWithLargeSkew"] = ["ball ID", "pocket ID"];
FunctionArgTypes["calculateCutAngle"] = ["number", "number"];
FunctionArgMeanings["calculateCutAngle"] = ["ball ID", "pocket ID"];
FunctionArgTypes["getCommand"] = ["number", "number"];
FunctionArgMeanings["getCommand"] = ["pocket ID", "ball ID"];
FunctionArgTypes["isPathBlocked"] = ["object", "object"];
FunctionArgMeanings["isPathBlocked"] = ["first position", "second position"];
FunctionArgTypes["extrapolatePoints"] = ["object", "object", "number"];
FunctionArgMeanings["extrapolatePoints"] = ["start point", "end point", "extension length"];
FunctionArgTypes["calculateProbability"] = ["object"];
FunctionArgMeanings["calculateProbability"] = ["shot command"];
FunctionArgTypes["calculateEndState"] = ["object"];
FunctionArgMeanings["calculateEndState"] = ["shot command"];
FunctionArgTypes["calculateCTP"] = ["object", "number", "object"];
FunctionArgMeanings["calculateCTP"] = ["shot command", "ball ID", "target point"];
FunctionArgTypes["dist2"] = ["object", "object"];
FunctionArgMeanings["dist2"] = ["first point", "second point"];
FunctionArgTypes["weaponIsBetter"] = ["number", "number"];
FunctionArgMeanings["weaponIsBetter"] = ["weapon type 1", "weapon type 2"];

FunctionArgTypes["isShellBlockedAtPos"] = ["number", "number", "string"];
FunctionArgMeanings["isShellBlockedAtPos"] = ["column", "row", "color"];
...
const functionNames = new Set([
  ...
  'getCutAngle', 'calculateCutAngle', 'calculateSidePocketSkew', 'calculateEndState', 'calculateCTP',  'calculateProbability', 'calculateProbability2', 'extrapolatePoints'
]);
...
function markPropertyNameValue(n) {
  if (!n) return;
  if (n.type == "Property") {
    n.key.type = "property name";
    if (n.value.type == "Literal") {
      n.value.type = "value for property " + n.key.codeStr;
    }
  }
}
...
function fillInSeparators(n) {
  ...
  switch (n.type) {
    ...
    case 'BlockStatement': {
      if (n.separators.length > 0)  n.separators[0].type = "opening '{' for code block";
      n.separators[n.separators.length-1].type = "closing '{' for code block";
      break;
    }
    case 'VariableDeclaration': {
      if (n.separators.length > 0)  n.separators[0].type = "variable kind";
      break;
    }
    case 'CallExpression': {
    ...
    case 'Property': {
      if (n.separators.length > 0) n.separators[0].type = "property assignment operator";
      break;
    }
    case 'AwaitExpression': {
      if (n.separators.length > 0) n.separators[0].type = "keyword 'await'";
      break;
    }
    case 'AssignmentExpression': {
      if (n.separators.length > 0)  n.separators[0].type = "assignment operator '='";
      break;
    }    
    case 'ForStatement': {
      for (let j=0; j<n.separators.length; j++) {
    ...
    case 'BinaryExpression': {
      if (n.separators[0] && n.separators[0].trim) {
        const op = n.separators[0].trim();
        if (["+", "-", "*", "/"].includes(op)) {
          n.separators[0].type = "number or string operator";  
        } else {
          n.separators[0].type = "comparison operator";
        }
      } else {
        if (n.separators.length > 0) n.separators[0].type = "binary operator";
      }
      break;
    }
    case 'LogicalExpression': {
      if (n.separators.length > 0) n.separators[0].type = "logical operator";
      break;
    }
    case 'UpdateExpression': {
      if (n.separators.length > 0)  n.separators[0].type = "update operator";
      break;
    }
    case 'IfStatement': {
      for (let j=0; j<n.separators.length; j++) {
      ...
      if (n.separators.length > 0) n.separators[0].type = "keyword 'if'";
      if (n.separators.length > 1) n.separators[1].type = "closing ')' for if";
      break;
    }
  }
}
...
function traverseAST(node, func, code) {
  ...
  for (let k=0; k<keylist.length; k++) {
    ...
    const p = node[keylist[k]];
    if (p == null) continue;
    // console.log("p.__proto__.constructor.name " + p.__proto__.constructor.name);
    if (!NonNodeType.includes(p.__proto__.constructor.name)) {
...
function addChildrenNodeList(node, nodelist) {
  node.childrenNodes = [];
  for (let k=0; k<nodelist.length; k++) {
    const n = nodelist[k];
    if (n.parent == node) {
      node.childrenNodes.push(n);
      addChildrenNodeList(n, nodelist);
    }
  }
...
function getPrevNode2(node) {
  const n = getPrevNode(node);
  if (!n) {
    debugger;
  }
  //if (n && n.CodeStr && n.codeStr.trim() == "") {
  if (n.codeStr.trim() == "") {
    return getPrevNode(n);
  }
  return n;
}
...
function getPrevNode(node) {
  ...
  for (let k=0; k<parent.childrenNodes.length; k++) {
    const m = parent.childrenNodes[k];
    if (m.end == node.start) return m;
    // if (parent.childrenNodes[k] == node) {
    //   return parent.childrenNodes[k-1];
    // }
  }
...
function getLastLeafChildren(node) {
  if (!node.childrenNodes) return node;
  if (node.childrenNodes.length == 0) return node;
  let maxEnd = -1;
  let lastNode = null;
  for (let k=0; k<node.childrenNodes.length; k++) {
    const m = node.childrenNodes[k];
    if (m.end > maxEnd) {
      maxEnd = m.end; lastNode = m;
    }
  }

  // didn't find it
  return getLastLeafChildren(lastNode);
}
...
function getNextNode(node) {
  ...
  const parent = node.parent;
  if (!parent) return null;
  for (let k=0; k<parent.childrenNodes.length-1; k++) {
    const m = parent.childrenNodes[k];
    if (m.start == node.end) return m;
    // if (parent.childrenNodes[k] == node) {
    //   return parent.childrenNodes[k+1];
    // }
  }
    ...
    if (p.indexOf("###") > 0) {
      ...
      if (arrays.includes(parts[0])) {
        ...
        for (let j=0; j<n[parts[0]].length; j++) {
          ...
          let d = getEditDistance(m.codeStr.substring(0, 15), parts[1]);
          if (parts[0] == "properties") {
            let pname1 = m.codeStr.substring(0, m.codeStr.indexOf(":")).trim();
            if (m.codeStr.indexOf(":") < 0) {
              pname1 = m.codeStr.trim();
            }
            let pname2 = parts[1].substring(0, parts[1].indexOf(":")).trim();
            ...
            if (pname1 == pname2) {
              ...
              d = getEditDistance("", "92u3oijfawefkjasldkfjasdlfj");
            }
          } 
...
export default function getRichErrorMessages(cleanCode, errors, userCode) {
  // console.log("getRichErrorMessages");
  if (!userCode || userCode.trim() === '') return [];
  if (!cleanCode || cleanCode.trim() == "") return [];
  ...
  const answerCode = cleanUpComment(cleanCode);
  ...
  var variableList = initVarList;
  objectPropertiesList = {};
  traverseAST(userAST, getVariableDef, userCode);
  traverseAST(userAST, defineObjects, userCode);
  ...
  //printTime("t4");
  // mark property name and values
  traverseAST(answerAST, markPropertyNameValue, answerCode);
  traverseAST(userAST, markPropertyNameValue, userCode);
  //printTime("t5");
  ...
  //printTime("t7d");
  // make sure each node has a list of all children nodes
  addChildrenNodeList(userAST, userNodeList);
  addChildrenNodeList(answerAST, answerNodeList);
  ...
  errors.forEach((error) => {
    try {
      ...
      if (origMessage.indexOf("unexpected token") >= 0) {
        ...
        const userPrevNode = getPrevNode2(errorUserNode);
        if (userPrevNode) {
          const answerPrevNode = findNodeByPath(answerAST, userPrevNode.navipath);  
          if (!answerPrevNode) {
            const info = {
            ...
            /**
             * {aimx: 0  aimy: 2} ==> "aimy: 2" -> prev answer node is "aimx: 0", its next node is ","
             * {aimx:  , aimy: 2} ==> "," -> prev answer node is "aimx: 0", its next node is also ",", but it's really missing "0"
             * {aimx  0, aimy: 2} ==> "0" -> prev answer node is "aimx: 0", its next node is also ",", but it's really missing ":"
             * 
             * so logic: given prev answer node, if all its parts are presented in user code, then use its next node
             * otherwise, return the one part missing
             */

            let correctAnswerNode = getNextNode(answerPrevNode); // getLastLeafChildren(answerPrevNode); 
            if (!correctAnswerNode) return;
            if (answerPrevNode.childrenNodes.length > 0) {
              for (let j=0; j < answerPrevNode.childrenNodes.length; j++) {
                const an = answerPrevNode.childrenNodes[j];
                const un = findNodeByPath(userAST, an.navipath);
                if (!un || an.type != un.type) {
                  correctAnswerNode = an; break;
                }
              }
            } 

            let value = replaceAll(correctAnswerNode.codeStr, "\n", "").trim();
            if (errorUserNode.codeStr.trim().indexOf(value) == 0) {
              // not really missing! fall back to old message
              const info = {
                message: error.message,
                severity: "error",
                from: error.from,
            ...
            const info = {
              message: `Are you missing '${value}' before this point as ${ToPrint(correctAnswerNode.type)}?`,
              severity: "error",
              from: indToPos(errorUserNode.start, userCode),
              to: indToPos(errorUserNode.end, userCode)
            };
            pushToResult(info, updatedRe, newMessages);
          ...
          if (answerPrevNode.childrenNodes.length > userPrevNode.childrenNodes.length) {
            // look inside answer node for missing item
            const edits = getEditDistance(answerPrevNode.codeStr, userPrevNode.codeStr);
            for (let j=0; j<edits.length; j++) {
              const es = edits[j].split("_");
              if (Number(es[2]) > errorInd) continue;
              if (es[0] == "D" && es[1].replace("\n", "").replace(" ", "").replace("\t", "") != "") {
                // found what was missing!
                const correctAnswerNode = findNodeContainingInd(answerNodeList, answerPrevNode.start + Number(es[2]));
                let value = replaceAll(correctAnswerNode.codeStr, "\n", "");
                value = replaceAll(value, " ", "");
                value = replaceAll(value, "\t", "");
                //value = replaceAll(value, " ", "");
                if (correctAnswerNode.type.toLowerCase().indexOf("separator") >= 0 || correctAnswerNode.type.toLowerCase().indexOf("operator") >= 0) {
                  const info = {
            ...
            const correctAnswerNode = answerPrevNode; //getNextNode(answerPrevNode);
            let value = replaceAll(correctAnswerNode.codeStr, "\n", "");
            const info = {
              message: `Are you missing a '${value}' as ${ToPrint(correctAnswerNode.type)} here?`,
              severity: "error",
        ...
        for (let j=edits.length-1; j>=0; j--) {
          ...
          if (es[0] == "D" && es[1].replace("\n", "").replace(" ", "").replace("\t", "") != "") {
            ...
            let value = replaceAll(correctAnswerNode.codeStr, "\n", "");
            //value = replaceAll(value, " ", "");
            if (correctAnswerNode.type.toLowerCase().indexOf("separator") >= 0) {
              const info = {
                message: `Are you missing a '${value}' as ${ToPrint(correctAnswerNode.type)} here?`,
                severity: "error",
                from: indToPos(errorUserNode.start, userCode),
        ...
        */
        ...
        // const info = {
        //   message: `You need to provide a constant expression as a test, such as 'a == 2', 'b > 3', 'c <= d', 'd != f', etc.`,
        //   severity: "error",
        //   from: error.from,
        //   to: error.to
        // };
    ...
    preDefined.forEach((mod) => {
      ...
      matchArr.forEach((m) => {
        ...
        if ( isFunc && !mod.functions.has(name) || !isFunc && !mod.constants.has(name)) {
          ...
          const m2 = `. If you want to call the function ${mod.moduleName}.${name}, please append () after it`;
          let m1 = isFunc ? `function '${name}'` : `property '${name}'`;
          if (!isFunc && isReallyFunc) m1 += m2;
          const info = {
            message: `${mod.moduleName} does not have a member ${m1}. `,
            severity: "error",
            from: { line: i, ch: m.index },
            to: { line: i, ch: m.index + m.name.length }
          };
          if (simi !== '') {
            const m3 = isFunc ? 'function' : 'property';
            info.message += `Do you intend to write the ${m3} '${simi}'? `;
          }
          results.push(info);
        }
      });
...
function checkEachLine(userCode, userAST, fnames) {
  ...
  for (let lineNum = 0; lineNum < userCodeArr.length; lineNum += 1) {
    ...
    // check for loops
    if (line.indexOf('for(') >= 0 && line.indexOf("console.log") < 0 && line.indexOf("print(") < 0 && line.indexOf('forEach') < 0 && line.indexOf(' of ') < 0) {
...
const defineObjects = (node, codeStr) => {

  if (node.type == "AssignmentExpression") {
    if (!node.left || !node.left.codeStr) return;
    if (node.left.type == "MemberExpression" && node.left.codeStr.indexOf("[") < 0 ) {
      if (node.left.object.type == "Identifier" && node.left.property.type == "Identifier")
        objectPropertiesList[node.left.object.name][node.left.property.name] = "number";
    }
    return;
  }


  if (!node || !node.type || !node.init || !node.init.type || node.type !== "VariableDeclarator") return;
  if (node.init.type == "CallExpression") return;
  if (node.init.type == "ObjectExpression") {
    if (!objectPropertiesList[node.id.name]) {
      objectPropertiesList[node.id.name] = {};
    }
    for (let k=0; k<node.init.properties.length; k++) {
      const p = node.init.properties[k];
      objectPropertiesList[node.id.name][p.key.name] = "number";
    }

  }
};

const checkObjectProperty = (node, codeStr) => {
  if (node && node.type) {
    // console.log("node type " + node.type + " " + node.codeStr.substring(0, 30));
  }
  if (!node || !node.type || node.type !== "MemberExpression") return;
  
  if (node.codeStr.indexOf("colorTyp") > 0) {
    // debugger;
  }
  const propObj = objectPropertiesList[node.object.name];
  if (!propObj) return;
  const keys = Object.keys(propObj);
  if (!keys.includes(node.property.name)) {
    const linesafter = codeStr.substring(node.end);
    const p = linesafter.split("\n");
    if ( (p[0].indexOf("=") < 0 || p[0].indexOf("==") >= 0) && node.codeStr.indexOf("[") < 0 ) {
      // no "=" to the right, so this is not assigning property
      let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
      let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
      from.ch = from.pos;
      to.ch = to.pos;
      const info = {
        message: `'${node.property.name}' is not a property of ${node.object.name}.`,
        severity: "warning", from, to
      };
      moreErrors.push(info);
    }
  }
...
const checkBinaryLogicExpression = (node, codeStr) => {
  ...
  if (node.codeStr.indexOf("null") >= 0) return;
  const ops = ["<=", ">=", "<", ">", "=="];
  if (!ops.includes(node.operator)) return;
  if (node.codeStr.indexOf("MyTank >") >= 0) {
    // debugger;
  }
  if (node.left.type != 'Identifier' && node.right.type != 'Identifier') {
    if (node.left.codeStr.indexOf("Balls[") == 0 || node.left.codeStr.indexOf("Pockets[") == 0) {
      if (node.left.codeStr.indexOf(".") < 0 && node.right.codeStr.trim() != "null") {
        let from = absoluteToRelativePos(node.left.start, codeStr.split('\n'));
        let to = absoluteToRelativePos(node.left.end, codeStr.split('\n'));
        from.ch = from.pos;
        to.ch = to.pos;
        const info = {
          message: `'${node.left.codeStr}' is an object, so you can't use it to directly compare with '${node.right.codeStr}'. Are you trying to use some property of '${node.left.codeStr}' for comparison?`,
          severity: "warning", from, to
        };
        moreErrors.push(info);
        return; 
    ...
    if (node.right.codeStr.indexOf("Balls[") == 0 || node.right.codeStr.indexOf("Pockets[") == 0) {
      if (node.right.codeStr.indexOf(".") < 0 && node.left.codeStr.trim() != "null") {
        let from = absoluteToRelativePos(node.right.start, codeStr.split('\n'));
        let to = absoluteToRelativePos(node.right.end, codeStr.split('\n'));
        from.ch = from.pos;
        to.ch = to.pos;
        const info = {
          message: `'${node.right.codeStr}' is an object, so you can't use it to directly compare with '${node.left.codeStr}'. Are you trying to use some property of '${node.right.codeStr}' for comparison?`,
          severity: "warning", from, to
        };
        moreErrors.push(info);
        return; 
  ...
  for (let k=0; k<vkeys.length; k++) {
    const vtype = variableList[vkeys[k]];
    const v = vkeys[k];
    if (node.right && node.right.name == v && (vtype == "array" || vtype == "array of int") ) {
      let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
      let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
      from.ch = from.pos;
      to.ch = to.pos;
      const info = {
        message: `'${node.right.name}' is an ${vtype}, so you can't use it to directly compare with '${node.left.codeStr}'. Are you trying to read the number of elements in ${node.right.name} using '${node.right.name}.length'?`,
        severity: "warning", from, to
      };
      moreErrors.push(info);
      return; 
    ...
    if (node.left && node.left.name == v && (vtype == "object") ) {
      let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
      let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
      from.ch = from.pos;
      to.ch = to.pos;
      const info = {
        message: `'${node.left.name}' is an object, so you can't use it to directly compare with '${node.right.codeStr}'. Are you trying to use some property of '${node.left.codeStr}' for comparison?`,
        severity: "warning", from, to
      };
      moreErrors.push(info);
      return; 
    ...
    if (node.right && node.right.name == v && (vtype == "object") ) {
      let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
      let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
      from.ch = from.pos;
      to.ch = to.pos;
      const info = {
        message: `'${node.right.name}' is an object, so you can't use it to directly compare with '${node.left.codeStr}'. Are you trying to use some property of '${node.right.codeStr}' for comparison?`,
        severity: "warning", from, to
      };
      moreErrors.push(info);
      return; 
    ...
    if (node.left && node.left.name == v && (vtype == "array") ) {
      let from = absoluteToRelativePos(node.left.start, codeStr.split('\n'));
      let to = absoluteToRelativePos(node.left.end, codeStr.split('\n'));
      from.ch = from.pos;
      to.ch = to.pos;
      const info = {
        message: `'${node.left.name}' is an array, so you can't use it to directly compare with '${node.right.codeStr}'. Are you trying to use an item in '${node.left.codeStr}' or its length for comparison?`,
        severity: "warning", from, to
      };
      moreErrors.push(info);
      return; 
    ...
    if (node.right && node.right.name == v && (vtype == "array") ) {
      let from = absoluteToRelativePos(node.right.start, codeStr.split('\n'));
      let to = absoluteToRelativePos(node.right.end, codeStr.split('\n'));
      from.ch = from.pos;
      to.ch = to.pos;
      const info = {
        message: `'${node.right.name}' is an array, so you can't use it to directly compare with '${node.left.codeStr}'. Are you trying to use an item in '${node.right.codeStr}' or its length for comparison?`,
        severity: "warning", from, to
      };
      moreErrors.push(info);
      return; 
...
const getAAn = (tp) => {
  if (["array", "object"].includes(tp)) 
    return "an";
  else 
    return "a";
};

const checkArrayIndex = (node, codeStr) => {
  if (!node || !node.type || node.type !== "MemberExpression" || node.codeStr.indexOf("[") < 0) return;


  if (node.property.codeStr.trim() == "") {
    let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
    let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
    from.ch = from.pos;
    to.ch = to.pos;
    const info = {
      message: `To access any element in the array ${node.object.name}, you need to specify an integer index, such as '${node.object.name}[0]'.`,
      severity: "warning", from, to
    };
    moreErrors.push(info);    
    return;
  ...
  const tp = variableList[node.property.name];
  if (tp && tp != "unknown" && tp != "number") {
    let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
    let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
    from.ch = from.pos;
    to.ch = to.pos;
    const info = {
      message: `To access any element in the array ${node.object.name}, you need to specify an integer index, such as '${node.object.name}[0]'. '${node.property.name}' is ${getAAn(tp)} ${tp}.`,
      severity: "warning", from, to
    };
    moreErrors.push(info);    
    return;
...
const checkFunctionCallParam  = (node, codeStr) => {
  if (!node || !node.type || node.type !== "CallExpression") return;

  const funcName = node.callee.codeStr; // name
  const seq = ["first", "second", "third", "forth"];
  if (!FunctionArgTypes[funcName]) return;
  
  if (node.arguments.length < FunctionArgTypes[funcName].length ) {
    let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
    let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
    from.ch = from.pos;
    to.ch = to.pos;
    const info = {
      message: `The function ${funcName} expects ${FunctionArgTypes[funcName].length} arguments, but you have only provided ${node.arguments.length} arguments.`,
      severity: "warning", from, to
    };
    moreErrors.push(info);    
    return;
  ...
  for (let k=0; k<node.arguments.length; k++) {
    const arg = node.arguments[k];
    vtype = variableList[arg.name];
    if (!vtype) {
      if (arg.type == "MemberExpression") {
        if (arg.codeStr.indexOf("[") < 0 && arg.codeStr.indexOf(".") > 0) {
          if (arg.object && arg.object.name && objectPropertiesList[arg.object.name]) {
            vtype = objectPropertiesList[arg.object.name][arg.property.name];
          }
        }
        if (arg.codeStr.indexOf("[") > 0 && arg.codeStr.indexOf(".") > 0) {
          if (arg.object && arg.object.codeStr) {
            if (arg.object.codeStr.indexOf("Pockets[") == 0) {
              if (arg.property.codeStr == "inPocket") {
                vtype = "boolean";
              } else if (["x", "y", "ID", "colorType"].includes(arg.property.codeStr)) {
                vtype = "number";
              } else if (["dir"].includes(arg.property.codeStr)) {
                vtype = "string";
              }
            }
          }
        ...
        if (arg.codeStr.indexOf("[") > 0 && arg.codeStr.indexOf(".") < 0) {
          if (arg.object && arg.object.name) {
            if (arg.object.codeStr.indexOf("Pockets") == 0 || arg.object.codeStr.indexOf("Balls") == 0 || arg.object.codeStr.indexOf("Weapons") == 0) {
              vtype = "object";
            }
            if (arg.object.codeStr.indexOf("legalBallIDs") == 0 || arg.object.codeStr.indexOf("IDList") == 0) {
              vtype = "number";
            }
          }
        }
    ...
    var properArgType = FunctionArgTypes[funcName][k];
    if (vtype && vtype != "unknown" && properArgType) {
      if (vtype != properArgType) {
        let from = absoluteToRelativePos(node.start, codeStr.split('\n'));
        let to = absoluteToRelativePos(node.end, codeStr.split('\n'));
        from.ch = from.pos;
        to.ch = to.pos;
        const info = {
          message: `The ${seq[k]} parameter should be ${getAAn(properArgType)} ${properArgType} for the ${FunctionArgMeanings[funcName][k]}, but '${arg.codeStr}' is ${getAAn(vtype)} ${vtype}.`,
          severity: "warning", from, to
        };
        moreErrors.push(info);
        return; 
...
const funcReturnType = {
  ...
  getCallShotCommand: 'object',
  getCommand: 'object',
  getAimPosition: 'object',
  getRandomNumber: 'number',
  getRandomCommand: 'string',
  getLegalBallIDs: 'array',
  extrapolatePoints: 'object',
  isPathBlocked: 'bool',
  getAngleToSidePocket: 'number',
  getCutAngle: 'number',
  getSecondsLeft: 'number',
  dist2: 'number',
  calculateCutAngle: 'number',
  calculateSidePocketSkew: 'number',
  calculateEndState: 'object',
  calculateCTP: 'object',
  "Math.abs": 'number',
  // calculateEndState2: 'object', 
  calculateProbability2: 'number',
  calculateProbability: 'number',
  createAStarGraph: 'array',
  getNearestItemAStar: 'object',
  getShortestPathCmd: 'array',
  createNewGraph: 'array'
};
...
const getVariableDef = (node, codeStr) => {
  ...
  decs.forEach((d) => {
    ...
    if (!d.init) return;
    let type = '';
    //if (d.init.type === "AssignmentExpression" && d.codeStr.indexOf("targetBallPos") >= 0) {
    ...
    //   debugger;
    // }
    if (!d.init || !d.init.type) {
      if (id.toLowerCase().indexOf("command") >= 0) {
        type = "object"
      } else {
        type = 'unknown';
      }
      ...
      const funcName = d.init.callee.codeStr; // name
      if (funcName in funcReturnType) {
        type = funcReturnType[funcName];
      } else type = 'unknown';

      if (funcName && funcName.indexOf("get") == 0 && funcName.indexOf("Command") > 0) {
        type = "object";
        objectPropertiesList[id] = {
          probability: "number", aimx: "number", aimy: "number", strength: "number", targetBallID: "number", targetPocketID: "number"
        };
      } else if (funcName && funcName.indexOf("is") == 0) {
        type = "boolean";
        
      } else if (funcName && funcName.indexOf("createNewGraph") == 0) {
        type = "array";
      } else if (funcName && funcName.indexOf("calcDangerScores") == 0) {
        type = "array";
      } else if (funcName && funcName.indexOf("getShortestPathCmd") == 0) {
        type = "array";
        
      } else if (funcName && funcName.indexOf("extrapolatePoints") == 0) {
        objectPropertiesList[id] = {
          x: "number", y: "number"
        };
      }
      ...
      if (d.init.raw === "null") {
        ...
        if (d.init.raw.indexOf('.') < 0) type = 'number';
        else type = 'number';
      ...
      if (lit) {
        ...
        else if (lit.raw.indexOf('.') < 0) type = 'number';
        else type = 'number';
      ...
      // else if (obj === 'calculateEndState2') type = 'array of object';
      ...
      } else if (obj === 'Balls' || obj === 'Pockets' || obj === 'Tanks') {
        type = 'object';
        if (obj == "Balls") {
          objectPropertiesList[id] = {
            x: "number", y: "number", ID: "number", inPocket: "boolean", colorType: "number"
          };
        }
        if (obj == "Pockets") {
          objectPropertiesList[id] = {
        ...
        if (obj == "Tanks") {
          objectPropertiesList[id] = {
            c: "number", r: "number", color: "string", dir: "string", health: "number", isFrozen: "boolean", powerPoint: "number", reloadTime: "number", specialPower: "object", specialWeapon: "object", tankID: "number"
          };
        }
...
export function checkParenthesis(answerCodeRaw, userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
    ...
    for (let k=0; k<userCodeArr.length; k++) {
      ...
      if (parts.includes("function")) {
        lineType = "function parameter list";
      ...
      if (lineType != "") {
        ...
        if (line.indexOf("(") < 0) {
          ...
          // results.push({
          //   message: `Missing the ')' to pair with this '(' for the ${lineType}. `,
          //   severity: "error",
          //   from: start,
          //   to: end
          // });
...
export function checkFunctionCall(answerCodeRaw, userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
...
export function checkArrayAccess(answerCodeRaw, userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
...
export function moreErrorCheckAST(answerCodeRaw, userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
    ...
    const answerCode = cleanUpComment(answerCodeRaw);
...
export function simplifyCode(code, answerCode) {
  const uLines = code.split("\n");
  const aLines = answerCode.split("\n");
  let stopLine = 0;
  let stopK = Math.min(uLines.length, aLines.length);
  // find the last line that they differ: increase k
  for (let k=0; k < Math.min(uLines.length, aLines.length); k++ ) {
    const uline = uLines[uLines.length-1-k];
    const aline = aLines[aLines.length-1-k];
    if (uline.trim() != aline.trim()) {
      stopLine = aLines.length-1-k;
      stopK = k;
      break;
    }
  }

  // go down to find first function they share: decrease k
  let fullStopK = Math.max(0, stopK-1);
  for (; fullStopK >= 0; fullStopK --) {
    const line = uLines[uLines.length-1-fullStopK];
    if (line.indexOf("function") >= 0 && line.indexOf("(") > line.indexOf("function") && line.indexOf(")") > line.indexOf("(") && line.indexOf("{") > line.indexOf(")")) {
      fullStopK ++;
      break;      
    }
  }
  fullStopK = Math.max(0, fullStopK);
  let newCode = uLines.slice(0, uLines.length-fullStopK).join("\n");
  let newAnswerCode = aLines.slice(0, aLines.length-fullStopK).join("\n");

  // add a simplified version of all functions below
  for (let j = uLines.length-fullStopK; j < uLines.length; j++) {
    const line = uLines[j];
    if (line.indexOf("function") >= 0 && line.indexOf("(") > line.indexOf("function") && line.indexOf(")") > line.indexOf("(") && line.indexOf("{") > line.indexOf(")")) {
      newCode += "\n" + line + "\n}\n";
    }
  ...
  for (let j = aLines.length-fullStopK; j < aLines.length; j++) {
    const line = aLines[j];
    if (line.indexOf("function") >= 0 && line.indexOf("(") > line.indexOf("function") && line.indexOf(")") > line.indexOf("(") && line.indexOf("{") > line.indexOf(")")) {
      newAnswerCode += "\n" + line + "\n}\n";
    }
  ...
  // let commonCode = "";
  // for (let j=stopLine+1; j<uLines.length; j++) {
  //   const line = uLines[j];
  //   if (line.indexOf("function") >= 0 && line.indexOf("(") > line.indexOf("function") && line.indexOf(")") > line.indexOf("(") && line.indexOf("{") > line.indexOf(")")) {
  //     commonCode += line + "}\n";
  //   } else {
  ...
  return {newCode, newAnswerCode};


  // const blocks1 = getCodeBlocks(userCode);
  // const blocks2 = getCodeBlocks(answerCode);

  // const userFuncs = Object.keys(blocks1);
  // const answerFuncs = Object.keys(blocks2);

  // const commonFuncs = userFuncs.filter(value => answerFuncs.includes(value));

  // for (let k=0; k<commonFuncs)

}

export function moreErrorChecks(answerCodeRaw, userCode) {
  let results = [];
  try {
    userCode = cleanUpComment(userCode);
    ...
    traverseAST(userAST, defineObjects, astucode);
...
*/
export function checkBracketErrors(answerCodeRaw, userCode) {
  let results = [];
  try {
    // const origUserCode = userCode;
    userCode = cleanUpComment(userCode, false);
    ...
    // let answerCode = cleanUpComment(answerCodeRaw, false);
...
export function checkKnownTypeErrors(answerCodeRaw, userCode) {
  try {
    ...
    traverseAST(userAST, checkFunctionCallParam, astucode);
    traverseAST(userAST, checkArrayIndex, astucode);

    traverseAST(userAST, checkObjectProperty, astucode);
