...
import getRichErrorMessages, { moreErrorChecks, checkBracketErrors, checkForLoopErrors } from './codeErrorCheck';
...
(function(mod) {
  ...
  function validator(text, options) {
    ...
    // let text2 = text.replace(asyncReg, "");
    // text2 = text2.replace(awaitReg, "");
    // JSHINT(text2, options, options.globals);
    // var errors = JSHINT.data().errors, result = [];

    

    // if (errors) parseErrors(errors, result, editor);
    ...
    //   if (moreErrs) moreErrs.forEach((r) => { result.push(r); });
    // }
    // return result;
  }
  ...
  function parseErrors(errors, output, editor) {
    ...
    if (window.userChat && window.scenario && editor) {
      ...
      if (results) results.forEach((r) => { output.push(r); });
    } else {
