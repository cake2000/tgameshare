...
const keyWords2 = new Set([
  ...
  'getAimPosition', 'getNewCommand', 'sendTankOrder', 'MoveRight', 'getRandomNumber', "getRandomCommand", 'extrapolatePoints', "CandidateBallList", 'calculateProbability', 'console', 'MyID', 'world', 'isPathBlocked', 'getCutAngle', 'calculateCutAngle', 'calculateSidePocketSkew', 'getAngleToSidePocket',
  'calculateEndState', 'calculateCTP', 'calculateEndState2', 'calculateProbability2', 'getDistance', 'Victor', 'getFirstBallTouched', 'Boundaries', "TOP_Y", "BOTTOM_Y", "LEFT_X", "RIGHT_X", 'debugger',
]);
...
const functionNames = new Set([
  ...
  'getCutAngle', 'calculateCutAngle', 'calculateSidePocketSkew', 'calculateEndState', 'calculateCTP', 'calculateEndState2', 'calculateProbability', 'calculateProbability2', 'extrapolatePoints'
]);
...
const funcReturnType = {
  ...
  getCutAngle: 'float',
  calculateCutAngle: 'float',
  calculateSidePocketSkew: 'float',
...
export function checkParenthesis(userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
    ...
    for (let k=0; k<userCodeArr.length; k++) {
      ...
      if (lineType != "") {
        ...
        if (line.indexOf("(") < 0) {
          ...
          // search for the ")" in next 2 lines as well
          // let lineall = line;
          // if (k+1 < userCodeArr.length ) lineall += userCodeArr[k+1];
          // if (k+2 < userCodeArr.length ) lineall += userCodeArr[k+2];
           
          // if (line.indexOf("(") >= 0 && lineall.indexOf(")") < lineall.indexOf("(")) {
...
export function checkFunctionCall(userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
...
export function checkArrayAccess(userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
...
export function moreErrorCheckAST(userCode) {
  let results = [];
  try {
    if (!userCode || userCode.trim === '') return [];
...
export function moreErrorChecks(userCode) {
  let results = [];
  try {
    userCode = cleanUpComment(userCode);
...
*/
export function checkBracketErrors(userCode) {
  let results = [];
  try {
    // const origUserCode = userCode;
    userCode = cleanUpComment(userCode, false);
...
*/
export function checkForLoopErrors(userCode) {
  try {
...
export function checkKnownTypeErrors(userCode) {
  try {
