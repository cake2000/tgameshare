...
(function(mod) {
  ...
  var DIFF_DELETE = -1;                                                            // 68
var DIFF_INSERT = 1;                                                             // 69
var DIFF_EQUAL = 0;   
  ...
  function markChanges(editor, diff, type, marks, from, to, classes) {
    ...
    let code = editor.options.value;
    if (editor.options.readOnly) {
      code = editor.options.orig;
    }
    ...
    for (var i = 0; i < diff.length; ++i) {
      ...
      if (tp == DIFF_EQUAL) {
        ...
        pending = true
        // if (tp == type && !( type == DIFF_INSERT && str.trim() == "" )) {
        if (tp == type) {
          var end = moveOver(pos, str, true);
          var a = posMax(top, pos), b = posMin(bot, end);
          if (!posEq(a, b)) {
            const codeline = allLines[pos.line];
            if (codeline.trim().indexOf("//") == 0) {
              // ignore diff in comments
            } else if (codeline.indexOf("print") < 0 && (type == DIFF_INSERT || type == DIFF_DELETE) && str.trim() == "") {
  ...
  function getDiff(a, b, ignoreWhitespace) {
    // if (!dmp) dmp = new window.diff_match_patch();
