...
april 10:
1. new lesson 19
2. OK review new master and add task on trello
3. new lesson 20
4. add to slides

new todo:
0. gold coins!
  a. if you practice yourself, you earn 1 coin every time
  b. if you practice online, the host can set up a wager up to 5 coins
  c. spending coins:
     10000 coins can get one month free for pro account
     100 coins allow you to unlock a new table layout or a new cue stick
1. OK change timer to use setInterval setupTimerUpdate and only runs at host and send to remote so it is synced
2. OK all tab events handled in playgame.js
 a. beforeunload
 b. unload
 c. blur: lose focus, run as usual!
 d. visibilityChange: switch off tab
2. OK when tab switches off, show pop up at remote site saying opponent has switched off tab, and will lose game after 1 minute, and show a timer update. when tab switch back resume normal game
3. OK handle when tab is closed
4. OK when one player quits game in network game
5. xx while you are waiting: CS history, facts, math models, interesting statistics
6. DONE can't go into a room if anyone has exited?
7. DONE bot name for 2 ai release and also shown
8. avatar
9. OK replay same game:
  a. if anyone player left the room, the game is over, and the other play can only choose to exit game
  b. if both players are in room, then when game is over,
    b1: guest side can "exit game" or "wait for replay"
    b2: host side can "exit game" or "replay game"
10. OK adjust pocket location for get shot command
11. DONE tutorial for isBlocked for getBallPlacement for one ball 2 pockets and getAimPosition with additional parameter
12. DONE tutorial for getBallPlacement using isBlocked and search
13. DONE test forum
14. DONE show user id with release name
15. DONE pocketing star

what's missing:
1. DONE simpler auto search
2. DONE game over: screen, button and disable play
3. DONE help button -> instruction and rules, and remove "Guides"
4. DONE probability
5. DONE side spin
6. DONE exit lose game message box
7. make sure tutorials works
*/
...
let url = "";
let world = {};
let Balls, Pockets, Cushions, BallDiameter, TableHeight, TableWidth, CushionWidth, PlayerInfo, Boundaries;
const resolveProbs = {};
...
const TakeCallShot = async function() {
  try {
    ...
    sendCommand(CMD_ERROR_IN_WORKER, 'Error found when taking call shot so using default shot to shoot at a random point', e.stack);
...
const PathPoolGame = function (gameSetup) {
  ...
  this.setupConfig = () => {
    ...
    const cfg = {
      TrueWidth: 2600, TrueHeight: 1265
    };
    ...
    cfg.tableCenterY = cfg.TrueHeight - cfg.tableH/2 -  cfg.metalBorderThick;
  ...
  this.createController = () => {
    ...
    const GameController = function () {
      ...
      this.setState = (s, p) => {
        ...
        const activeID = gameSetup.activePlayerInfo.ID;
        ...
        if (this.gameState ==  WAIT_FOR_BREAK_STATE) {
          if (gameSetup.gameType == GAME_TYPE.TESTING || gameSetup.gameType == GAME_TYPE.REPLAY) {
            ...
            if (gameSetup.activePlayerInfo.isLocal && gameSetup.activePlayerInfo.playerType != "AI") {
              gameSetup.config.showMessage(`Player ${gameSetup.activePlayerInfo.playerID} to place cue ball on left of head string then click 'Place'.`, 10, gameSetup.placeButton.position.x - gameSetup.placeButton.width/2, gameSetup.placeButton.position.y + gameSetup.config.tableH * 0.07);
            } else {
              gameSetup.config.showMessage(`Player ${gameSetup.activePlayerInfo.playerID} to place cue ball on left of head string.`);
        ...
        if (this.gameState == BREAK_CUEBALL_IN_HAND_STATE && Number(s) == BREAK_SHOT_STATE) {
          ...
          if (gameSetup.gameType == GAME_TYPE.TESTING || gameSetup.gameType == GAME_TYPE.REPLAY) {
            ...
            // this line runs on both sides so no need to send message all
            gameSetup.config.showMessage(`Player ${gameSetup.activePlayerInfo.playerID} to take the break shot`);
          }
        }
        ...
        if (isTesting) {
          ...
          if (gameSetup.activePlayerInfo.playerType == "AI") {
            gameSetup.networkHandler.sendCommandToAll({
              ...
              // bbbbb
              // c: "NewActivePlayerInfo", t: gameSetup.currentCycleTime, w: `${id}_${CALL_SHOT_STATE}`
            });
          } else {
            gameSetup.networkHandler.sendCommandToAll({
...
ReportEndOfTest();
      ...
      this.onStrikeButtonClick = function () {
        ...
        // const strength = gameSetup.speedMeterBall.value / unitScale; // unit difference?
        // const spinMultiplier = 0 - SPIN_M * gameSetup.spinMeterBar.value;
      ...
      this.enterState = (p) => {
        ...
        switch (this.gameState) {
          ...
          case BREAK_CUEBALL_IN_HAND_STATE: { // dedicated for human player state!
            ...
            gameSetup.cuestick.visible = false;
            // config.sendMessageAll(`${gameSetup.activePlayerInfo.playerID} to place cue ball`, 0);
          ...
          case BREAK_SHOT_STATE: {
            //config.sendMessageAll(`${gameSetup.activePlayerInfo.playerID} to take break shot`, 1);
          ...
          case CALL_SHOT_STATE: {
            ...
            // config.sendMessageAll(`${gameSetup.activePlayerInfo.playerID} to take a call shot`, 0);
          ...
          case GAME_OVER_STATE: {
            ...
            gameSetup.sounds.backgroundmusic.stop();
            //gameSetup.showWinner(gameSetup.activePlayerInfo.ID, gameSetup.activePlayerInfo.playerID);
            // if (0 && gameSetup.isHost && gameSetup.gameType != GAME_TYPE.TOURNAMENT) {

            //   gameSetup.showModalMessage(`The winner is player ${gameSetup.activePlayerInfo.ID}: ${gameSetup.activePlayerInfo.playerID} !`, p[5], MODAL_EXITORREPLAY);
            // } else {
            //   gameSetup.showModalMessage(`The winner is player ${gameSetup.activePlayerInfo.ID}: ${gameSetup.activePlayerInfo.playerID} !`, p[5], MODAL_EXITGAME);
            // }
            ...
            if (isTournament) {
              gameSetup.showModalMessage(`The winner is player ${gameSetup.activePlayerInfo.ID}: ${gameSetup.activePlayerInfo.playerID} !`, p[5], MODAL_NOBUTTON);
              PoolActions.reportGameResult(gameSetup.room._id, gameSetup.activePlayerInfo.playerUserId);
              const params = {
                modalIsOpen: true,
              ...
              PoolActions.reportGameResult(gameSetup.room._id, gameSetup.activePlayerInfo.playerUserId);
              gameSetup.showModalMessage(`Autorun: the winner is player ${gameSetup.activePlayerInfo.ID}: ${gameSetup.activePlayerInfo.playerID} !`, p[5], MODAL_EXITGAME);
      ...
      for (let playerID = 1; playerID <= 2; playerID++) {
        ...
        modalItems.push({
          ...
          content: 'black ball was pocketed though it\'s not the target',
        });
      ...
      this.handleAllStopped = () => {
        ...
        const activeID = gameSetup.activePlayerInfo.playerID;
        ...
        const otherID = gameSetup.waitingPlayerInfo.playerID;
        ...
        switch (Number(this.gameState)) {
          ...
          case BREAK_SHOT_STATE: {
            ...
            if (failReason == "") {
              ...
              config.sendMessageAll(`Great shot. Player ${gameSetup.playerInfo[newID].playerID} will make a call shot on any RED or YELLOW ball now.`);
              // config.sendMessageAll(`${gameSetup.activePlayerInfo.playerID} to take a call shot`);
              this.setNewPlayerState(newID, newState);
            } else {
              // invalid break shot: switch to opponent to redo it
              config.sendMessageAll(`Invalid break shot: ${failReason}. Player ${otherID} will make a break shot now.`);
          ...
          case CALL_SHOT_STATE: {
            ...
            if (gameSetup.ballsByID[BLACK_BALL_ID] && gameSetup.ballsByID[BLACK_BALL_ID].body.inPocketID !== null) { // black ball pocketed! serious event!
              ...
              if (gameSetup.targetBallID !== BLACK_BALL_ID) {
                ...
                if (!isTesting) {
                  config.sendMessageAll(`Unfortunately ${failReason}. Player ${otherID} has won the game!`);
                ...
                if (gameSetup.ballsByID[CUE_BALL_ID].body.inPocketID !== null) {
                  ...
                  if (!isTesting) {
                    config.sendMessageAll(`Unfortunately ${failReason}. Player ${otherID} will place the ball on the table and take a call shot.`);
                  ...
                  if (!isTesting) {
                    ...
                    config.sendMessageAll(`Bravo! Player ${activeID} has won the game!`);
            ...
            if (gameSetup.ballsByID[CUE_BALL_ID].body.inPocketID !== null) {
              ...
              if (!isTesting) {
                config.sendMessageAll(`Unfortunately ${fr}. Player ${otherID} will place the ball on the table and take a call shot.`);
                this.togglePlayer(CUEBALL_IN_HAND);
              }
              break;
            } else {
              ...
              if (gameSetup.activePlayerInfo.chosenColor != null) {
                if (gameSetup.firstBallTouchedByCueball.colorType !== gameSetup.activePlayerInfo.legalColor) {
                  ...
                  if (!isTesting) {
                    // console.log(`Unfortunately for player ${activeID}: first touch color is ${gameSetup.firstBallTouchedByCueball.colorType} but legal color is  ${gameSetup.activePlayerInfo.legalColor}. Player ${otherID} will place the ball on the table and take a call shot.`);
                    // debugger;
                    config.sendMessageAll(`Unfortunately ${fr}. Player ${otherID} will place the ball on the table and take a call shot.`);
                    this.togglePlayer(CUEBALL_IN_HAND);
                  }
              ...
              if (gameSetup.targetPocketID == null || gameSetup.targetBallID == -1) {
                ...
                if (!isTesting) {
                  ...
                  if (gameSetup.activePlayerInfo.chosenColor != null) {
                    ...
                    this.defineLegalColorForNonActivePlayer();
                    config.sendMessageAll(`${fr}. Player ${otherID} will take a call shot on any ${ColorTypeString2[gameSetup.waitingPlayerInfo.legalColor]} ball.`);
                  } else {
                    config.sendMessageAll(`${fr}. Player ${otherID} will take a call shot on any red or yellow ball.`);
                  }
                  // config.sendMessageAll(`${fr}. Player ${otherID} will take a call shot now.`);
              ...
              if (gameSetup.ballsByID[gameSetup.targetBallID].body.inPocketID == gameSetup.targetPocketID) {
                ...
                if (gameSetup.activePlayerInfo.chosenColor == null) { // still open table so choose color
                  ...
                  config.sendMessageAll(`Player ${activeID} has chosen the ${ColorTypeString2[gameSetup.ballsByID[gameSetup.targetBallID].colorType]} color, and will continue to make a call shot.`);
                  ...
                  this.defineLegalColorForActivePlayer();
                  config.sendMessageAll(`Player ${activeID} will continue to make a call shot on any ${ColorTypeString2[gameSetup.activePlayerInfo.legalColor]} ball.`);
                }
                ...
                if (1 || gameSetup.someBallTouchedRailedAfter) {
                  ...
                  if (!isTesting) {
                    if (gameSetup.activePlayerInfo.chosenColor != null) {
                      this.defineLegalColorForNonActivePlayer();
                      config.sendMessageAll(`Target ball was not pocketed in the target pocket. Player ${otherID} will take a call shot on any ${ColorTypeString2[gameSetup.waitingPlayerInfo.legalColor]} ball.`);
                    } else {
                      config.sendMessageAll(`Target ball ${gameSetup.targetBallID} was not pocketed in target pocket ${gameSetup.targetPocketID}. Player ${otherID} will take a call shot on any red or yellow ball.`);
                    }
                    this.togglePlayer(CALL_SHOT_STATE);
                  }
                } else {
                  ...
                  if (!isTesting) {
                    config.sendMessageAll(`Unfortunately no ball hit the cushions after the cue ball's initial hit. Player ${otherID} will place the ball on the table and take a call shot.`);
                    this.togglePlayer(CUEBALL_IN_HAND);
                  }
                }
              }
      ...
      */
        ...
        if ( true ) {
          for (let j=0; j<allIDs.length; j++) {
            ...
            if (b.inPocketID == null && !b.isStopped) {
              if (Math.abs(b.velocity[0]) < 1000 && Math.abs(b.velocity[1]) < 1000) continue;
              // if (!gameSetup.innerRectangle2.contains(b.position[0], b.position[1])) {
                // if (gameSetup.innerRectangle.contains(b.position[0], b.position[1])) {
                  minTimeStep = 1 / 600; hasContact = true;
                  break;
                // }
              // }
            }
      ...
      this.updateGameWorld = () => {
        ...
        if (this.allowInput()) {
          ...
          for (let k=0; k<that.balls.length; k++) {
            ...
            // ball.body.ballIDLabel.visible = true; // aaaa test
      ...
      /* new method:
        1. calc prob given target ball and each pocket, pick the best
        2. fine tune angle to maximize pocketing probability
      */

      // this.runAutoSearch = () => {
      //   if (!this.allowInput() && !gameSetup.inAutoSearch) {
      //     console.log("stop search when input not allowed " + cueballInHand);
      //     return;
      //   }

      //   if (gameSetup.inAutoSearch) return;

      //   gameSetup.inAutoSearch = true;


      //   const doHideOverlay = () => {
      //     setTimeout(()=>{
      //       // make sure all mouse click events get through the system before we end the autosearch!
      //       gameSetup.inAutoSearch = false;
      ...
      //       if (gameSetup.gameType != GAME_TYPE.TESTING) {
      //         gameSetup.autoBtn.buttonMode = true;
      //         gameSetup.stage.interactive = true;
      //         gameSetup.stage.buttonMode = true;
      //         gameSetup.controlcontainer.interactive = true;
      //       }
      ...
      //   let origDir = gameSetup.cueballDirection.clone();
      //   const targetID = gameSetup.autoSearchTargetID;
      //   const targetBall = gameSetup.ballsByID[targetID];
      //   const cueball = gameSetup.ballsByID[0];
      //   let maxProb = 0.001; let maxPocketID = null;
      //   const targetBallPos = new Victor(targetBall.position.x, targetBall.position.y);


      //   if (0) {

      //     gameSetup.cueballDirection.x = 63.62876928547263;
      //     gameSetup.cueballDirection.y = -259.57424551165377;
      //     gameSetup.targetBallID = 2;
      //     const prob = getPocketProbability(2, 4);
      //     console.log("prob for " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y + " is " + prob);
      //     return;
      //   }

      //   if (0) {
      //     gameSetup.targetPocketID = 13;
      //     const pocketPos = gameSetup.tablePocket[gameSetup.targetPocketID].clone();
      //     const dirBallToPocket = pocketPos.clone().subtract(targetBallPos);
      //     const dirAimToBall = dirBallToPocket.normalize().scale(config.ballD);
      //     const aimPos = targetBallPos.clone().subtract(dirAimToBall);
      //     console.log("aimPos " + aimPos.x + " " + aimPos.y);
      //     gameSetup.cueballDirection.x = aimPos.x - cueball.position.x;
      //     gameSetup.cueballDirection.y = aimPos.y - cueball.position.y;
      //     gameSetup.targetBallID = targetID;
      //     const prob = getPocketProbability(targetID, gameSetup.targetPocketID);
      //     console.log("prob for " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y + " is " + prob);
      //     return;
      //   }

      //   // get init probability for each of 6 pockets!
      ...
      //     const prob = getPocketProbability(targetID, pocketID);
      //     // console.log("prob for " + pocketID + " is " + prob);
      //     // console.log("prob for " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y + " is " + prob);
      //     if (prob > maxProb) {
      //       maxProb = prob; maxPocketID = pocketID;
      //       dirx = gameSetup.cueballDirection.x;
      //       diry = gameSetup.cueballDirection.y;
      //     }
      //   }

      //   if (maxPocketID == null) {
      //     gameSetup.cueballDirection.x = origDir.x;
      //     gameSetup.cueballDirection.y = origDir.y;
      //     config.sendMessageAll("No good shot found! Try a different speed?", 1);

      //     doHideOverlay();
      //     return;
      //   } else {
      //     // set init dir to that pocket
      //     gameSetup.cueballDirection.x = dirx;
      //     gameSetup.cueballDirection.y = diry;
      //     origDir.x = dirx; origDir.y = diry;
      //   }

      //   // have locked in target pocket and ball
      //   // just need to refine to improve probability!

      //   const pocketPos = gameSetup.tablePocket[maxPocketID].clone();
      //   const dirBallToPocket = pocketPos.clone().subtract(targetBallPos);
      //   const distBallToPocket = dirBallToPocket.length();
      //   let pocketD = config.cornerPocketD;
      //   if (maxPocketID == 1 || maxPocketID == 4)
      //     pocketD = config.sidePocketD;
      //   const fullAngle = Math.atan2(pocketD * 0.8, distBallToPocket);

      //   const N = 20;
      //   maxProb = 0.01; let maxDirx = null; let maxDiry = null;
      //   for (let k=0-N/2; k<=N/2; k++) {
      //     const ang = fullAngle / N * k;
      //     const dirBallToPocketNew = dirBallToPocket.clone().rotate(ang);
      //     // console.log("run simulation " + k + " " + ang + " with dir " + dirBallToPocketNew.x + " " + dirBallToPocketNew.y);
      //     const aimPos = targetBallPos.clone().subtract(dirBallToPocketNew.clone().normalize().scale(config.ballD));
      //     gameSetup.cueballDirection.x = aimPos.x - cueball.position.x;
      //     gameSetup.cueballDirection.y = aimPos.y - cueball.position.y;

      //     // gameSetup.forecastG.moveTo(gameSetup.cueball.position.x, gameSetup.cueball.position.y);
      //     // gameSetup.forecastG.lineTo(aimPos.x, aimPos.y);
      //     // gameSetup.forecastG.lineTo(aimPos.x + dirBallToPocketNew.x, aimPos.y + dirBallToPocketNew.y);

      //     that.runSimulation(SIM_PROB, 0);
      //     if (targetID == gameSetup.targetBallID && gameSetup.pocketIDMap[maxPocketID] == gameSetup.targetPocketID) {
      //       // console.log("has pocketing at " + k +"!");
      //       gameSetup.targetBallID = targetID;
      //       gameSetup.targetPocketID = gameSetup.pocketIDMap[maxPocketID];
      //       const prob = getPocketProbability(gameSetup.targetBallID, gameSetup.targetPocketID);
      //       // console.log("prob for " + k + " is " + prob);
      //       // console.log("prob for " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y + " is " + prob);
      //       if (prob > maxProb) {
      //         maxProb = prob; maxDirx = gameSetup.cueballDirection.x; maxDiry = gameSetup.cueballDirection.y;
      //       }
      //     }
      //   }

      //   if (maxDirx == null) {
      //     config.sendMessageAll("Sorry no good shot found!", 1);
      //     gameSetup.cueballDirection.x = origDir.x;
      //     gameSetup.cueballDirection.y = origDir.y;
      //   } else {
      //     gameSetup.cueballDirection.x = maxDirx;
      //     gameSetup.cueballDirection.y = maxDiry;
      //   }

      //   doHideOverlay();
      //   return;

      //   const dx = gameSetup.ballsByID[targetID].position.x;
      //   const dy = gameSetup.ballsByID[targetID].position.y;
      //   const cx = gameSetup.cueball.body.position[0];
      //   const cy = gameSetup.cueball.body.position[1];
      //   const dist = Math.sqrt((dx-cx)*(dx-cx) +(dy-cy)*(dy-cy));
      //   const halfAngle = Math.atan2(config.ballD, dist);

      //   gameSetup.autoSearchStepSize = halfAngle / 4;
      //   const saveTargetBallID = targetID;
      //   const saveTargetPocketID = maxPocketID;

      //   for (let round=0; round < 20; round ++) {
      //     gameSetup.cueballDirection.x = origDir.x;
      //     gameSetup.cueballDirection.y = origDir.y;
      //     console.log("\n\n\nonly look at target ball " + targetID);
      //     // try 1 left and 1 right,  to calculate pocketing prob
      //     // if either one is higher, pick highest and go
      //     // if neither is higher, then reduce step size by half
      //     console.log("orig dir at " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y);
      //     const origProb = getPocketProbability(saveTargetBallID, saveTargetPocketID);
      //     gameSetup.cueballDirection = origDir.clone().rotate(gameSetup.autoSearchStepSize);
      //     console.log("up dir at " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y);
      //     const upProb = getPocketProbability(saveTargetBallID, saveTargetPocketID);
      //     gameSetup.cueballDirection = origDir.clone().rotate(0 - gameSetup.autoSearchStepSize);
      //     console.log("low dir at " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y);
      //     const downProb = getPocketProbability(saveTargetBallID, saveTargetPocketID);
      //     console.log("prob orig up down " + origProb + " " + upProb + " " + downProb);

      //     gameSetup.targetBallID = saveTargetBallID;
      //     gameSetup.targetPocketID = saveTargetPocketID;

      //     if (origProb >= upProb && origProb >= downProb) {
      //       gameSetup.cueballDirection = origDir.clone();
      //       if (gameSetup.autoSearchStepSize <= Math.PI / 900) {
      //         // first see if we have a target already
      //         that.runSimulation(SIM_PROB, 0, saveTargetBallID);
      //         if (saveTargetBallID == gameSetup.targetBallID && saveTargetPocketID == gameSetup.targetPocketID) {
      //           // good! jsut return!
      //           console.log("no need to run! " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y);
      //         } else {
      //           probCache = {};
      //           gameSetup.minSkew = 10000;
      //           gameSetup.cueballDirection.x = origDir.x;
      //           gameSetup.cueballDirection.y = origDir.y;
      //           gameSetup.targetBallID = saveTargetBallID;
      //           gameSetup.targetPocketID = saveTargetPocketID;
      //           console.log("\n\n\nfinal sim run ! " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y);
      //           getPocketProbability(saveTargetBallID, saveTargetPocketID);
      //           const strength = gameSetup.speedMeterBall.value / 100 * MAX_SPEED / unitScale; // unit difference?
      //           const spinMultiplier = 0 - 2.5 * gameSetup.spinMeterBar.value;
      //           const rot = (gameSetup.minSkew) * (Math.PI / gameSetup.Randomness) * (strength * (1 + Math.abs(spinMultiplier / 2.5)));
      //           const newDir = origDir.clone().rotate(rot);
      //           gameSetup.cueballDirection.x = newDir.x;
      //           gameSetup.cueballDirection.y = newDir.y;
      //           console.log("minSkew dir: " + gameSetup.cueballDirection.x + " " + gameSetup.cueballDirection.y);
      //         }
      //         return;
      //       } else {
      //         gameSetup.autoSearchStepSize = gameSetup.autoSearchStepSize * 0.9;
      //         if (upProb < 1 && downProb < 1) {
      //           gameSetup.autoSearchStepSize = gameSetup.autoSearchStepSize * 0.3;
      //         }
      //         console.log("reduce step size to " + gameSetup.autoSearchStepSize);
      //       }
      //     } else {
      //       if (upProb > downProb) {
      //         console.log("move up to search");
      //         gameSetup.cueballDirection = origDir.clone().rotate(gameSetup.autoSearchStepSize);
      //         origDir = gameSetup.cueballDirection.clone();
      //       } else {
      //         console.log("move down to search");
      //         gameSetup.cueballDirection = origDir.clone().rotate(0 - gameSetup.autoSearchStepSize);
      //         origDir = gameSetup.cueballDirection.clone();
      //       }
      //     }
      ...
      const PlaceCueBallFromHand = function() {
        // debugger;
        gameSetup.controller.setActivePlayerInfo(`${0}_${CUEBALL_IN_HAND}_-1_-100000_-100000`);
        // gameSetup.controller.updateWorld();
      ...
      const TakeBreakShot = function () {
        ...
        gameSetup.controller.setActivePlayerInfo(`${0}_${BREAK_SHOT_STATE}_-1_-100000_-100000`);
      };

      const TakeCallShot = function () {
        //me.enterState(GameState.BreakShot, gameSetup.playerInfo1);
        gameSetup.controller.setActivePlayerInfo(`${0}_${CALL_SHOT_STATE}_-1_-100000_-100000`);
      };
...
// }
  ...
  this.loadTextures = () => {
    const textures = [];
    // console.log("in loadTextures");
    // textures.push('/images/smallclothhalf.jpg');
    // this.load.spritesheet('button', '/images/plus_minus.png', 31, 31);
    let cueStick = _.get(gameSetup, 'cueStick.imageSrc.main', '/images/poolstickpurple.png');
    if (gameSetup.gameType == GAME_TYPE.REPLAY) {
      cueStick = "/images/Lightning_Strike.png";
    }
    textures.push(cueStick);
    if (gameSetup.gameType === GAME_TYPE.MATCH || gameSetup.gameType === GAME_TYPE.TOURNAMENT) {
        const cueStickOpponent = _.get(gameSetup, 'cueStickOpponent.imageSrc.main', '/images/poolstickpurple.png');
        if (cueStickOpponent !== cueStick) {
          textures.push(cueStickOpponent);
        }
    }
    ...
    const poolTableBig = _.get(gameSetup, 'poolTable.imageSrc.main', '/images/new_pool_table_trim_hd2_darker.png');
    const poolTableSmall = _.get(gameSetup, 'poolTable.imageSrc.small', '/images/new_pool_table_trim_hd_darker.png');
    ...
    const initFunction = () => {
      if (!PIXI.loader.resources["/images/bluebackground4.jpg"]) {
  ...
  this.makeBall = (x, y, color, ID) => {
    ...
    this.loadFramedSpriteSheet(`/images/new${ballfile}ballrolling.png`, ballfile, 41, 41, (frames) => {
  ...
  this.addOpponentPoolStick = () => {
    ...
    const cueStick = _.get(gameSetup, 'cueStickOpponent.imageSrc.main', '/images/poolstickpurple.png');
  ...
  this.addHelpQuestionMark = () => {
    ...
    bg2.position.x = (config.TrueWidth - config.tableW - 2 * config.metalBorderThick)/4;
    bg2.position.y = (config.TrueHeight - (config.tableH + 2 * config.metalBorderThick)) / 2;
    bg2.anchor.set(0.5, 0.5);
    bg2.interactive = true;
    bg2.buttonMode = true;
    gameSetup.stage.addChild(bg2);
  ...
  this.addCWNew = () => {
    ...
    gameSetup.nextAutoTarget = (dir=1) => {
      ...
      // let targetID = -1;
      // let msg = "Please aim at a ball first.";
      // if (b !== null) {
      //   if (gameSetup.activePlayerInfo.chosenColor != null) {
      //     if (gameSetup.firstBallTouchedByCueball.ball.colorType != gameSetup.activePlayerInfo.legalColor) {
      //       targetID = -1;
      //       msg = `Please aim at a ${gameSetup.activePlayerInfo.legalColor} ball first.`;
      //     } else {
      //       targetID = b.ID;
      //     }
      //   } else {
  ...
  this.addQuitButton = () => {
    ...
    bg.position.x = config.tableCenterX - config.tableW/2 - config.metalBorderThick;
  ...
  this.addHitButton = () => {
    ...
    bg2.position.x = (config.tableCenterX + config.tableW/2 + config.metalBorderThick);
    bg2.position.y = (config.TrueHeight - config.tableH - 2 * config.metalBorderThick) / 2;
  ...
  this.addBackGround = () => {
    const config = gameSetup.config;
    // new new way: draw a green table top graphics
    const bg = new PIXI.Sprite(PIXI.loader.resources["/images/bluebackground4.jpg"].texture);

    bg.scale.set(config.TrueWidth/1398, config.TrueHeight / 680); // will overflow on bottom
  ...
  this.addTableImage = () => {
    const poolTableBig = _.get(gameSetup, 'poolTable.imageSrc.main', '/images/new_pool_table_trim_hd2_darker.png');
    let poolTableSmall = _.get(gameSetup, 'poolTable.imageSrc.small', '/images/new_pool_table_trim_hd_darker.png');
    if (gameSetup.gameType == GAME_TYPE.REPLAY) {
  ...
  this.drawPockets = () => {
    ...
    gameSetup.tablePocket[0] = new Victor(config.tableCenterX - config.tableW / 2 + shift * 1.1, config.tableCenterY - config.tableH / 2 + shift);
    gameSetup.tablePocket[2] = new Victor(config.tableCenterX + config.tableW / 2 - shift * 1.1, config.tableCenterY - config.tableH / 2 + shift);
    gameSetup.tablePocket[3] = new Victor(config.tableCenterX + config.tableW / 2 - shift * 1.1, config.tableCenterY + config.tableH / 2 - shift);
    gameSetup.tablePocket[5] = new Victor(config.tableCenterX - config.tableW / 2 + shift * 1.1, config.tableCenterY + config.tableH / 2 - shift);
  ...
  this.drawTableCushionBars = () => {
    ...
    //   this.addBoxBody(world2, 10000, 10000, config.tableCenterX + config.tableW/2 + 5000 - 200, config.tableCenterY);
    //   this.addBoxBody(world2, 10000, 10000, config.tableCenterX - config.tableW/2 - 5000 + 200, config.tableCenterY);
    // }
    ...
    // borderg.drawRect(config.tableCenterX - config.tableW / 2 + config.cornerPocketD / 2 - config.pocketShift,
    //   config.tableCenterY - config.tableH / 2 - config.metalBorderThick,
    //   config.tableW / 2 - sizeratio * config.sidePocketD / 2 - config.cornerPocketD / 2,
    ...
    // borderg.drawRect(config.tableCenterX + sizeratio * config.sidePocketD / 2,
    //   config.tableCenterY - config.tableH / 2 - config.metalBorderThick,
    //   config.tableW / 2 - sizeratio * config.sidePocketD / 2 - config.cornerPocketD / 2,
    //   config.metalBorderThick);
    // borderg.drawRect(config.tableCenterX + sizeratio * config.sidePocketD / 2,
    ...
    // borderg.drawRect(config.tableCenterX - config.tableW / 2 - config.metalBorderThick,
    //   config.tableCenterY - config.tableH / 2 + 1.2 * config.sidePocketD / 2,
    //   config.metalBorderThick, config.tableH - 1.12 * config.sidePocketD);
    // borderg.drawRect(config.tableCenterX + config.tableW / 2,
    //   config.tableCenterY - config.tableH / 2 + 1.2 * config.sidePocketD / 2,
    //   config.metalBorderThick, config.tableH - 1.12 * config.sidePocketD);
    ...
    // dotg.drawCircle(config.tableCenterX - config.tableW / 4, config.tableCenterY - config.tableH / 2 - config.metalBorderThick / 2, config.ballD / 3);
    // dotg.drawCircle(config.tableCenterX - config.tableW / 8, config.tableCenterY - config.tableH / 2 - config.metalBorderThick / 2, config.ballD / 3);
    // dotg.drawCircle(config.tableCenterX - 3 * config.tableW / 8, config.tableCenterY - config.tableH / 2 - config.metalBorderThick / 2, config.ballD / 3);
    ...
    // let centerx = config.tableCenterX - config.tableW / 2 - config.metalBorderThick / 2;
    // dotg.drawCircle(centerx, config.tableCenterY - config.tableH / 4, config.ballD / 3);
    // dotg.drawCircle(centerx, config.tableCenterY, config.ballD / 3);
    // dotg.drawCircle(centerx, config.tableCenterY + config.tableH / 4, config.ballD / 3);

    // centerx = config.tableCenterX + config.tableW / 2 + config.metalBorderThick / 2;
    // dotg.drawCircle(centerx, config.tableCenterY - config.tableH / 4, config.ballD / 3);
    // dotg.drawCircle(centerx, config.tableCenterY, config.ballD / 3);
    // dotg.drawCircle(centerx, config.tableCenterY + config.tableH / 4, config.ballD / 3);
  ...
  const loadPlayRightBall = (cc, x, y, color) => {
    ...
    that.loadFramedSpriteSheet(`/images/new${ballfile}ballrolling.png`, ballfile, 41, 41, (frames) => {
      const ball = new PIXI.extras.AnimatedSprite(frames);
      ball.scale.set(gameSetup.config.ballD * 1.5 / 41);
    ...
    world.on("beginContact", (evt) => {
      ...
      if (b.name == "ball" && b2.name == "ball") {
        ...
        if (b.ID == 0) {
          ...
          if (gameSetup.firstBallTouchedByCueball === null) {
            // console.log("setting gameSetup.firstBallTouchedByCueball = b2; color " + b2.colorType);
  ...
  this.setupForecast = () => {
    ...
    // bbbbb
    // gameSetup.cueballDirection = new Victor(0.9764549137578125, -0.21572158306072012);
      ...
      if (room.offer1 == "" && room.answer1 == "") {
        ...
        if (gameSetup.isHost) {
          ...
          gameSetup.tryReconnectTimer = setTimeout(() => {
            // check for reconnect
            if (!gameSetup.networkHandler) {
              console.log("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nnot connected after 5 sec! try to reconnect!");
  ...
  this.applyRollingFriction = (bodylist, isRealRun) => {
    ...
    for (let j=0; j<allIDs.length; j++) {
      ...
      if (b.inPocketID == null && !b.isStopped) {
        ...
        if (b.ID === 0) {
          that.counter ++;
          // console.log(that.counter + ": after adjustVAV , " + v.x + "," + v.y + ",av," + av.x + "," + av.y + ",pos," + b.x + "," + b.y);
        }
    ...
    if (gameSetup.controller.gameState > 0 || gameSetup.gameType == GAME_TYPE.TESTING) {
      ...
      if (simType != SIM_DRAW) {
        ...
        if (true || showAutoMark) {
          ...
          if (gameSetup.autoBtn) gameSetup.autoBtn.visible = true;
        } else {
