...
class TrajectoryPoolComponent extends React.Component {
  componentDidMount() {
    ...
    tableg.lineStyle(0);
    if (isTesting) {
      tableg.beginFill(0x310d54, 1); // light green game background: 83c3a1, 74c1f2 gamewrapper
    } else {
      tableg.beginFill(0x110000, 1); // dark red game background
    }

    tableg.drawRect(0, 0, TrueWidth, TrueHeight + 10);
    tableg.endFill();
  ...
  tryResize() {
    ...
    if (showControls) {
      ...
      stage.position.set(0 - (TrueWidth - wfulltable) / 2 * stage.scale.x, 0 - (TrueHeight - 110 - hfulltable + 5) * stage.scale.y);
    }
    renderer.render(stage);
