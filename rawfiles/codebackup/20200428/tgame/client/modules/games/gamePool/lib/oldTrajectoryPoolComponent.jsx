...
import TrajectoryPool from "./oldtrajectorypoolgame.js";

const TrueHeight = 1265;
const TrueWidth = 2600;
...
class TrajectoryPoolComponent extends React.Component {
  componentDidMount() {
  ...
  tryResize() {
    ...
    const oldnewratio = TrueWidth / 1600; // new vs old true width
    ...
    if (showControls) {
      ...
      stage.position.set(0 - (TrueWidth - wfulltable) / 2 * stage.scale.x, 0 - (TrueHeight - hfulltable + 5) * stage.scale.y);
    }
    renderer.render(stage);
  ...
  createPoolGameNew() {
    const {
      scenario, isReplay, isTesting, activeGame, room, config, isProfessionalUser, reacthistory
    } = this.props;
    ...
    if (isReplay) {
      ...
      gameSetup.isHost = room.playerInfo[0].userId === userId || gameSetup.gameType === GAME_TYPE.AUTORUN || gameSetup.gameType === GAME_TYPE.BATTLE;
    ...
    const gameObj = new TrajectoryPool(gameSetup);
...
export default TrajectoryPoolComponent;