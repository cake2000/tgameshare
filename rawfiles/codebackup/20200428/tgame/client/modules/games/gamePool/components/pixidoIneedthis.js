  ...
  PIXI.DisplayObject.prototype = {
    ...
    */
    generateTexture (resolution, scaleMode, renderer) {
      ...
      const renderTexture = new PIXI.RenderTexture(bounds.width | 0, bounds.height | 0, renderer, scaleMode, resolution);
    ...
    */
    _generateCachedSprite () {
      ...
      if (!this._cachedSprite) {
        const renderTexture = new PIXI.RenderTexture(bounds.width, bounds.height);
      ...
      set (value) {
        if (Array.isArray(value)) {
          ...
          for (let i = 0; i < value.length; i++) {
            const filterPasses = value[i].passes;

            for (let j = 0; j < filterPasses.length; j++) {
...
*/
  PIXI.Sprite.prototype._renderCanvas = function(renderSession, matrix) {
    ...
    if (this.texture.valid) {
      const resolution = this.texture.baseTexture.resolution / renderSession.resolution;
      ...
      let dx = (this.texture.trim) ? this.texture.trim.x - this.anchor.x * this.texture.trim.width : this.anchor.x * -this.texture.frame.width;
      let dy = (this.texture.trim) ? this.texture.trim.y - this.anchor.y * this.texture.trim.height : this.anchor.y * -this.texture.frame.height;
      ...
      const cw = this.texture.crop.width;
      const ch = this.texture.crop.height;
      ...
      if (this.tint !== 0xFFFFFF) {
        ...
        const cx = this.texture.crop.x;
        const cy = this.texture.crop.y;
  ...
  PIXI.SpriteBatch.prototype._renderCanvas = function(renderSession) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      ...
      if (child.rotation % (Math.PI * 2) === 0) {
        ...
        const tx = (childTransform.tx * renderSession.resolution) + renderSession.shakeX;
        const ty = (childTransform.ty * renderSession.resolution) + renderSession.shakeY;
 ...
 */
  PIXI.canUseNewCanvasBlendModes = function() {
    ...
    const canvas = PIXI.CanvasPool.create(this, 6, 1);
  ...
  PIXI.CanvasPool = {
    ...
    */
    create (parent, width, height) {
      const idx = PIXI.CanvasPool.getFirst();
      let canvas;
    ...
    */
    getFirst () {
      const pool = PIXI.CanvasPool.pool;
...
*/
  PIXI.compileProgram = function(gl, vertexSrc, fragmentSrc) {
    const fragmentShader = PIXI.CompileFragmentShader(gl, fragmentSrc);
    const vertexShader = PIXI.CompileVertexShader(gl, vertexSrc);
...
*/
  PIXI.PixiShader.prototype.init = function() {
    ...
    const program = PIXI.compileProgram(gl, this.vertexSrc || PIXI.PixiShader.defaultVertexSrc, this.fragmentSrc);
...
*/
  PIXI.PixiFastShader.prototype.init = function() {
    ...
    const program = PIXI.compileProgram(gl, this.vertexSrc, this.fragmentSrc);
...
*/
  PIXI.WebGLBlendModeManager.prototype.setBlendMode = function(blendMode) {
    ...
    const blendModeWebGL = PIXI.blendModesWebGL[this.currentBlendMode];
 ...
 */
  PIXI.WebGLStencilManager.prototype.bindGraphics = function(graphics, webGLData, renderSession) {
    ...
    let projection = renderSession.projection,
...
*/
  PIXI.WebGLSpriteBatch.prototype.setContext = function(gl) {
    ...
    const shader = new PIXI.PixiShader(gl);
...
*/
  PIXI.WebGLSpriteBatch.prototype.renderTilingSprite = function(sprite) {
    const texture = sprite.tilingTexture;
    ...
    const offsetX = sprite.tilePosition.x / (w * sprite.tileScaleOffset.x);
    const offsetY = sprite.tilePosition.y / (h * sprite.tileScaleOffset.y);

    const scaleX = (sprite.width / w) / (sprite.tileScale.x * sprite.tileScaleOffset.x);
    const scaleY = (sprite.height / h) / (sprite.tileScale.y * sprite.tileScaleOffset.y);
    ...
    const tint = sprite.tint;
    const color = (tint >> 16) + (tint & 0xff00) + ((tint & 0xff) << 16) + (sprite.worldAlpha * 255 << 24);
    ...
    let i = this.currentBatchSize * 4 * this.vertSize;
...
*/
  PIXI.WebGLFilterManager.prototype.pushFilter = function(filterBlock) {
    ...
    const projection = this.renderSession.projection;
    const offset = this.renderSession.offset;
    ...
    const filter = filterBlock.filterPasses[0];
    ...
    const filterArea = filterBlock._filterArea;// filterBlock.target.getBounds();///filterBlock.target.filterArea;
...
*/
  PIXI.WebGLFilterManager.prototype.popFilter = function() {
    const gl = this.gl;
    const filterBlock = this.filterStack.pop();
    let filterArea = filterBlock._filterArea;
    let texture = filterBlock._glFilterTexture;
    const projection = this.renderSession.projection;
    const offset = this.renderSession.offset;
    ...
    if (filterBlock.filterPasses.length > 1) {
      ...
      let outputTexture = this.texturePool.pop();
      ...
      for (let i = 0; i < filterBlock.filterPasses.length - 1; i++) {
        const filterPass = filterBlock.filterPasses[i];
    ...
    const filter = filterBlock.filterPasses[filterBlock.filterPasses.length - 1];
    ...
    if (this.filterStack.length === 0) {
      ...
      const currentFilter = this.filterStack[this.filterStack.length - 1];
    ...
    const x = filterArea.x - offsetX;
    const y = filterArea.y - offsetY;
...
*/
  PIXI.WebGLFilterManager.prototype.applyFilterPass = function(filter, filterArea, width, height) {
    // use program
    const gl = this.gl;
    let shader = filter.shaders[gl.id];
 ...
 */
  PIXI.CanvasTinter.getTintedTexture = function(sprite, color) {
    const canvas = sprite.tintedTexture || PIXI.CanvasPool.create(this);
 ...
 */
  PIXI.CanvasTinter.tintWithPerPixel = function(texture, color, canvas) {
    ...
    const rgbValues = PIXI.hex2rgb(color);
    let r = rgbValues[0],
      g = rgbValues[1],
      b = rgbValues[2];
 ...
 */
  PIXI.CanvasTinter.checkInverseAlpha = function() {
    const canvas = new PIXI.CanvasBuffer(2, 1);
 ...
 */
  PIXI.BaseTexture.prototype.unloadFromGPU = function() {
    ...
    for (let i = this._glTextures.length - 1; i >= 0; i--) {
      const glTexture = this._glTextures[i];
      const gl = PIXI.glContexts[i];
 ...
 */
  PIXI.Texture.fromCanvas = function(canvas, scaleMode) {
    const baseTexture = PIXI.BaseTexture.fromCanvas(canvas, scaleMode);
 ...
 */
  PIXI.RenderTexture.prototype.renderCanvas = function(displayObject, matrix, clear) {
    ...
    const realResolution = this.renderer.resolution;
 ...
 */
  PIXI.RenderTexture.prototype.getCanvas = function() {
    if (this.renderer.type === PIXI.WEBGL_RENDERER) {
      const gl = this.renderer.gl;
      const width = this.textureBuffer.width;
      const height = this.textureBuffer.height;
      ...
      const tempCanvas = new PIXI.CanvasBuffer(width, height);
      const canvasData = tempCanvas.context.getImageData(0, 0, width, height);
 ...
 */
  PIXI.AbstractFilter.prototype.syncUniforms = function() {
    for (let i = 0, j = this.shaders.length; i < j; i++) {
...
// constructor
  ...
  PIXI.Strip.prototype._renderStrip = function(renderSession) {
    ...
    const drawMode = this.drawMode === PIXI.Strip.DrawModes.TRIANGLE_STRIP ? gl.TRIANGLE_STRIP : gl.TRIANGLES;
  ...
  PIXI.Strip.prototype._renderCanvasDrawTriangle = function(context, vertices, uvs, index0, index1, index2) {
    const textureSource = this.texture.baseTexture.source;
    ...
    let u0 = uvs[index0] * textureWidth,
      u1 = uvs[index1] * textureWidth,
      u2 = uvs[index2] * textureWidth;
    let v0 = uvs[index0 + 1] * textureHeight,
 ...
 */
  PIXI.Strip.prototype.renderStripFlat = function(strip) {
    ...
    for (let i = 1; i < length - 2; i++) {
      ...
      let x0 = vertices[index],
        x1 = vertices[index + 2],
        x2 = vertices[index + 4];
      let y0 = vertices[index + 1],
        y1 = vertices[index + 3],
        y2 = vertices[index + 5];
...
*/
  PIXI.TilingSprite.prototype._renderCanvas = function(renderSession) {
    ...
    const sessionBlendMode = renderSession.currentBlendMode;
    ...
    let tw = this._width / tileScale.x;
    let th = this._height / tileScale.y;
...
*/
  PIXI.TilingSprite.prototype.generateTilingTexture = function(forcePowerOfTwo, renderSession) {
    ...
    let targetWidth = this._frame.sourceSizeW || this._frame.width;
    let targetHeight = this._frame.sourceSizeH || this._frame.height;
    ...
    let w = texture.crop.width;
    let h = texture.crop.height;
