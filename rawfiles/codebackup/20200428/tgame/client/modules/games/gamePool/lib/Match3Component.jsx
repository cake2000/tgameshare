...
import Match3 from '../lib/match3game';

const TrueHeight = 969;
const TrueWidth = 1607;
...
class Match3Component extends React.Component {
  ...
  componentDidMount() {
    ...
    tableg.lineStyle(0);
    if (isTesting) {
      tableg.beginFill(0x005500, 1); // black game background
    } else {
      tableg.beginFill(0x550000, 1); // dark red game background
    }

    tableg.drawRect(0, 0, TrueWidth, TrueHeight);
    tableg.endFill();
  ...
  tryResize() {
    ...
    const w2mazes = 1561; // width of just 2 mazes
    const h2mazes = 702;


    const showControls = !isTesting;
    // showControls = 0; // aaaa
    if (showControls) {
      ...
      stage.scale.x =  w2 / TrueWidth;
      stage.scale.y =  stage.scale.x; //h2 / 1000;
      stage.position.set(0, 0);
    } else {
      const ratio = w2mazes / h2mazes;
      ...
      // const r = (w2mazes / 108); // table width is 9 feet = 108 inch
      //const metalBorderThick = 2.2 * r; // new table
      stage.scale.x =  w2 / w2mazes;
      stage.scale.y =  h2 / h2mazes; //h2 / 1000;
      stage.position.set(0 - (TrueWidth - w2mazes)/2 * stage.scale.x, 0 - (TrueHeight - h2mazes + 5) * stage.scale.y);
    }
    renderer.render(stage);
  ...
  createMatch3Game() {
    const { scenario, isReplay, isTesting, activeGame, room, config, isProfessionalUser, reacthistory } = this.props;
    ...
    // const PathPoolGameSetupLib = require('../lib/Match3gamesetup.js');
    ...
    gameSetup.poolTable = _get(config, 'poolTable');
    gameSetup.cueStick = _get(config, 'cueStick');
    gameSetup.cueStickOpponent = _get(config, 'cueStickOpponent');
    ...
    if (isReplay) {
      ...
      gameSetup.isHost = room.playerInfo[0].userId === Meteor.userId() || gameSetup.gameType === GAME_TYPE.AUTORUN || gameSetup.gameType === GAME_TYPE.BATTLE;
    ...
    const gameObj = new Match3(gameSetup);
  ...
  render() {
    ...
    return (
      <div className="gameWrapper" style={{ height: '100%', width: '100%', position: 'relative', display: 'block', border: '1px solid #383737', 'flex': 1, 'flexDirection': 'column' }}>
        <div style={{ position: 'relative', height: '100%' }} ref={(thisDiv) => { component.gameCanvas = thisDiv; }} />
        <div style={{ backgroundColor: 'white', display: 'none', opacity: 1, position: 'absolute', top: '0px', width: '100%', height: '100%' }} id="chartcontainer" >
          <button style={{ backgroundColor: '#236bc2', position: 'absolute', zIndex: 100, color: '#ffffff', right: '10px' }} onClick={this.hideChartDiv.bind(this)} >
              Close
          </button>
          <div style={{ backgroundColor: 'white', position: 'absolute', zIndex: 10, top: '0px', width: '100%', height: '100%' }} id="datachart" />
        </div>
      </div>
...
export default Match3Component;
